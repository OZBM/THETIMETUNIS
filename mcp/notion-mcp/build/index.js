...
function markdownToBlocks(md) {
    ...
    let codeLines = [];
    const flushParagraphBuffer = (buffer) => {
        if (buffer.length === 0)
            return;
...
async function findChildPageByTitle(parentPageId, title) {
    ...
    for (const res of search.results) {
        if (res.object === "page" && res.parent?.type === "page_id" && res.parent.page_id === parentPageId) {
            ...
            const pageTitle = res.properties?.title?.title?.[0]?.plain_text ??
...
async function replacePageContent(pageId, blocks) {
    ...
    do {
        ...
        for (const blk of resp.results) {
            ...
            await notion.blocks.update({ block_id: blk.id, archived: true });
        }
    ...
    for (let idx = 0; idx < blocks.length; idx += chunkSize) {
        const slice = blocks.slice(idx, idx + chunkSize);
...
async function createPageUnderParent(parentPageId, title, children, options) {
    const properties = {
        title: [
            {
                type: "text",
    ...
    const page = await notion.pages.create({
        ...
        properties,
        icon: options?.icon ? { type: "emoji", emoji: options.icon } : undefined,
    ...
    if (children && children.length) {
        ...
        for (let idx = 0; idx < children.length; idx += chunkSize) {
            const slice = children.slice(idx, idx + chunkSize);
            await notion.blocks.children.append({ block_id: pageId, children: slice });
        }
    }
    return pageId;
...
async function upsertPage(parentPageId, title, blocks, opts) {
...
server.tool("set_page_properties", {
    ...
    return {
        content: [{ type: "text", text: JSON.stringify({ pageId: resp.id, last_edited_time: resp.last_edited_time }, null, 2) }],
    };
});
// upload_markdown tool
server.tool("upload_markdown", {
    ...
    if (!resolvedTitle) {
        ...
        if (h1Line)
            resolvedTitle = h1Line.replace(/^#\s+/, "").trim();
    }
    if (!resolvedTitle)
        resolvedTitle = path.basename(absPath).replace(/\.[^.]+$/, "").replace(/[-_]/g, " ").trim();
    let pageId;
    if (upsert) {
